...
pub fn post_response<T>(cookies: &mut Cookies, data: T, target_path: String) -> PostResponse
where
    T: Serialize,
{
    ...
    cookie.set_path(target_path.clone());
    cookies.add(cookie);

    let mut header = HeaderMap::new();
    header.insert(header::LOCATION, HeaderValue::from_str(&target_path.clone()).unwrap());
